...
<html>
    <head>
        ...
        <title>View Bookings | Receptionist</title>
...
$sql = "SELECT room_no, room_type, cost, bookedby FROM rooms WHERE status=1";
$result = mysqli_query($conn, $sql);

if (mysqli_num_rows($result) > 0) {
    // output data of each row
    echo "<table cellpadding='10'><tr><th>Room Number</th><th>Room Type</th><th>Cost</th><th>Booked By</th></tr>";
    while($row = mysqli_fetch_assoc($result)) {
        $no=$row["room_no"];
        echo "<tr><td>" . $no . "</td><td>" . $row["room_type"]. "</td><td>" . $row["cost"]. "</td><td>" . $row["bookedby"] . "</td></tr>";
    }
   echo "</table>";
} else {
...
?> 
    ...
    <a href="recephome.php"><button id = 'back1221' type="button">Back to Home Page</button></a> 
</body>
</html>