...
$sql = "SELECT room_no, room_type, cost FROM rooms WHERE status=1";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
    	$no=$row["room_no"];
        echo "<tr><td>" . $no . "</td><td>" . $row["room_type"]. "</td><td>" . $row["cost"]. "</td><td><a href='checkedout.php?roomid=$no'><button type='button' > Checkout </button></a></td></tr>";
    }
   echo "</table>";
} else {
...
?> 
	...
	<a href="recephome.php"><button type="button">Back to Home Page</button></a> 
    <br>
    <br>
    <hr>
</body>
